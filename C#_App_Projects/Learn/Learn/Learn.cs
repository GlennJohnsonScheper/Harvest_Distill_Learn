...
namespace Learn
{
	public partial class Form1 : Form
	{
		...
		static Boolean logging = false; // true;
		...
		int SaveScrollScanLine = -1; // 2022-05-24 ready to use again, let ^G undo last HOME or END.

		static string keypressAccumulator = string.Empty;
		static int AccuPowerTen = 1;
		static int AccuDigitRun = 0;
		static string searchPattern = "asdf"; // the headline marker between files inserted by DISTILL program
		...
		static volatile string caption = "Learn"; // later, a per-mille
		...
		void SettingsReset()
		{
			...
			// 2022-03-06 Let's start RESTORE=NORMAL, tall and slim:
			// uncommenting: No, myFormWinState does not exist...
			// myFormWinState = (uint)FormWindowState.Normal; // Normal == 0
			// Rather, I had to set .Normal in the VS Form Designer itself.
			...
			// 2022-06-06 lower the top to just below my Hebrew-English tachistoscope display:
			myFormLocation = new Point(Screen.PrimaryScreen.WorkingArea.Width * 23/100, Screen.PrimaryScreen.WorkingArea.Height * 9/100);
			myFormSize = new Size(Screen.PrimaryScreen.WorkingArea.Width * 60/100, Screen.PrimaryScreen.WorkingArea.Height * 87/100);
		...
		void Form1_Shown(object sender, EventArgs e)
		{
			if (logging) log("in Form1_Shown");
			{
				...
				// No need here to do -- myBackingAndFontAvailableMutex.WaitOne(); -- it has been owned by main thread from Form1 ctor until now
				{
					...
					// No, I am tired of this feature,
					// Instead, be ready to immediately receive a CONTROL-V paste....
					...
					*/
					...
					// 2022-03-12 Let's just save that one more CONTROL-V user keystroke,
					// cloning from the ^V pasting code:
			...
			// 2022-07-04 Since adding the auto-CONTROL-V at start,
			// it may not paint the first screen until a keystroke.
			// FWIW, Copy this bool true to before starting thread:
						...
						case Keys.End: // Bare or Any End = Stop scrolling, go to end of document
							{
								...
								if (ScrolledToScanLine > 0
								    && ScrolledToScanLine < TotalScanLines)
								{
									SaveScrollScanLine = ScrolledToScanLine; // set during END key
								}
						...
						case Keys.G: // Control + 'G' = Green Background
							{
								...
								// SaveScrollScanLine = -1; // 2022-05-24 ready to use again, let ^G undo last HOME or END.
								if(SaveScrollScanLine != -1)
                                {
									ScrolledToScanLine = SaveScrollScanLine;
									SaveScrollScanLine = -1;
								}
								else
								{

									//2022-03-06 let '^G' also "GO" to 0-99 % of document.
									// which % was previously keyed in, as with ^F and ^R.

									// 2022-03-20 Change from % to per-mil

									if (keypressAccumulator.Length > 0
										&&
										keypressAccumulator.Length <= 3)
									{
										int percentNowPermil = -1;
										if (char.IsDigit(keypressAccumulator[0]))
										{
											percentNowPermil = keypressAccumulator[0] - '0';
											if (keypressAccumulator.Length >= 2)
											{
												if (char.IsDigit(keypressAccumulator[1]))
												{
													percentNowPermil *= 10;
													percentNowPermil += keypressAccumulator[1] - '0';
												}
												else
												{
													percentNowPermil = -1;
												}
												if (keypressAccumulator.Length == 3)
												{
													if (char.IsDigit(keypressAccumulator[2]))
													{
														percentNowPermil *= 10;
														percentNowPermil += keypressAccumulator[2] - '0';
													}
													else
													{
														percentNowPermil = -1;
										...
										// cloning from ^A code:
										if (percentNowPermil != -1)
										{
											ScrolledToScanLine = TotalScanLines * percentNowPermil / 1000;
											PaintingRequired = true;
										}
									}
								}
		...
		void Form1_KeyUp(object sender, KeyEventArgs e)
		{
			...
			this.Text = caption; // 2022-03-20 new way
		}

		void Form1_KeyPress(object sender, KeyPressEventArgs e)
		{
			if (logging) log("in Form1_KeyPress");
			this.Text = caption; // 2022-03-12 new way
			if (e.KeyChar >= ' '
			    && e.KeyChar <= '~')
			{
		...
		// This delegate stuff did work, but occasionally now
		// the entire app becomes unresponsive, must kill it.

		// 2022-03-12 no longer calling this delegate stuff.

		// By 2022-03-06 this is no more mystery,
		// just need google to master the syntax.

		// I intend to call this from non-GUI thread to update Form1.Text:

		// Declare a delegate.
		delegate void DelGato(string s);

		// Declare a method with the same signature as the delegate.
		private void GUIThreadUpdateForm1Text(string s)
		{
			this.Text = s;
		}

		public void OtherThreadUpdateForm1Text(string s)
		{
			// Create an instance of the delegate.
			DelGato delicato = new DelGato(GUIThreadUpdateForm1Text);

			object[] args = { s };
			this.Invoke(delicato, args);
		}
		...
		void hotPaintWorker()
		{
			...
			for (; !myFormIsClosing; )
			{
				...
				if (PaintingRequired)
				{
					...
					string newText = "Learn";
					if (TotalScanLines > 0)
					{
						// 2022-04-22 Huh!?An 11 MB file flipped caption from (+)188 to -181 or so! Go Long!
						newText = (ScrolledToScanLine * 1000L / TotalScanLines).ToString("d3") + "%";
					}
					caption = newText;

					// 2022-03-12 no longer calling this delegate stuff.

					// 2022-03-06 I know now how to handle that, by a pinvoke.
					// Going to find an example.... Well, I thought it had a p.
					// kills app ... OtherThreadUpdateForm1Text(newText);
					...
					if (direction < 0)
					{
						...
						for (int i = NowAt - 1; i >= 0; i--) // reverse find
						{
							    ...
							    && VerifyContainsDetails(WrappedLineArray[i].ToLower(), searchPattern))
							{
								...
								// 2018-11-11 Finally fixing on Crash if Ctl-R is at top
								if(ScrolledToScanLine < 0)
									ScrolledToScanLine = 0;
